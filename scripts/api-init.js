...
const DOCS_URL = 'http://localhost:3000/dist/docs.json';

// Use for production development
// const DOCS_URL = 'https://cdn.jsdelivr.net/npm/@stories-js/core@0.2.0/dist/docs.json';

/**
 * This script will mass generate all the docs for the Stories Framework components.
 * It will parse the contents of the docs.json file and generate a markdown file for each component.
 *
 ...
 *
 * The auto generated files will be gitignored and continued to be generated from the published docs.json.
...
(async function () {
  ...
  let addons = [], coreComponents = [];
  components.forEach(comp => {
    if (comp.tag.startsWith('str-addon')) {
      addons.push(comp);
    } else {
      coreComponents.push(comp);
    }
  });

  // Generate documentation for core components
  generate(coreComponents, './docs/api')

  // Generate documentation for addons
  generate(addons, './docs/addons')

  console.log('Finished one-time migration of component docs');
})();

function generate(components, dir) {
  const names = components.map((component) => component.tag.slice(0));
  // matches all relative markdown links to a component, e.g. (../button)
  COMPONENT_LINK_REGEXP = new RegExp(`\\(../(${names.join('|')})/?(#[^)]+)?\\)`, 'g');

  components.forEach((comp) => {
    const compTag = comp.tag.slice(0);
    writePage(comp, dir);
...
function writePage(page, dir) {
  ...
  // const dir = './docs/api';
  const path = `${dir}/${page.tag.slice(0)}.md`;
...
function renderFrontmatter({ tag }) {
  ...
  const demoPath = `api/${tag.slice(0)}/index.html`;
...
import Props from '@site/static/auto-generated/${tag.slice(0)}/props.md';
import Events from '@site/static/auto-generated/${tag.slice(0)}/events.md';
import Methods from '@site/static/auto-generated/${tag.slice(0)}/methods.md';
import Parts from '@site/static/auto-generated/${tag.slice(0)}/parts.md';
import CustomProps from '@site/static/auto-generated/${tag.slice(0)}/custom-props.md';
import Slots from '@site/static/auto-generated/${tag.slice(0)}/slots.md';
